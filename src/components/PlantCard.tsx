...
export const PlantCard: React.FC<PlantCardProps> = ({ plant, category, className = '' }) => {
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>) => {
    e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(
      plant.name
    )}&background=f3f4f6&color=374151&size=400&bold=true`;
  };
  ...
  return (
    ...
    >
      <div className="card card-interactive group">
        {/* Image */}
        <div className="aspect-video bg-gray-100 overflow-hidden max-w-[800px] mx-auto">
          <img
            src={plant.image}
            alt={plant.name}
        ...
        <div className="p-6">
          ...
          <div className="flex gap-2 mb-4">
            {category && (
              <span
                className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full border ${getCategoryColor(
                  plant.category
                )}`}
              >
            ...
            <span
              className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full border ${getDifficultyColor(
                plant.difficulty
              )}`}
            >
